...
class InputEmbedding(nn.Module):
    ...
    def __init__(self, d_model: int, vocab_size: int):
        ...
        self.register_buffer("scale", torch.sqrt(torch.tensor(d_model, dtype=torch.float32))) # here we are registering the scale variable as a buffer because it is involved in the forward pass but it is not learned, hence registering it as a buffer is helpful as it will be moved to the same device and dtype as the model
    ...
    def forward(self, x : torch.Tensor) -> torch.Tensor:
        return self.embedding(x) * self.scale # in the original transformer they multiply the embeddings by the square root of the embedding dimension