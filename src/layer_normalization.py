...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        mean = x.mean(
            dim=-1, keepdim=True
        )  # we compute the mean but keep the dimension
        std = x.std(
            dim=-1, keepdim=True, unbiased=False
        )  # by default the Bessel's correction is applied to the std for statistical estimation which is not the case for layer norm
